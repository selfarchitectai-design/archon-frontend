...
interface Component { name: string; status: "green" | "yellow" | "red"; latency: number; message: string; }
interface Health { overall: "green" | "yellow" | "red"; score: number; components: Component[]; timestamp: string; }
...
const Alert = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><circle cx="12" cy="17" r="1"/></svg>;
...
const Badge = ({ s }: { s: "green" | "yellow" | "red" }) => {
  const m: Record<string, { c: string; l: string; I: React.FC }> = {
    ...
    yellow: { c: "bg-yellow-500/20 text-yellow-400 border-yellow-500/40", l: "Warn", I: Alert },
...
const Gauge = ({ v, c }: { v: number; c: string }) => {
  const sz = 130, sw = 8, r = (sz - sw) / 2, circ = r * 2 * Math.PI, off = circ - (v / 100) * circ;
  return (
...
const Spark = ({ d, c }: { d: number[]; c: string }) => {
  const h = 45, w = 110, mx = Math.max(...d), mn = Math.min(...d), rg = mx - mn || 1;
...
const Node = ({ n, s, I, x, y }: { n: string; s: string; I: React.FC; x: string; y: string }) => {
  ...
  return (
    <div className="absolute flex flex-col items-center hover:scale-110 transition z-10" style={{ left: x, top: y, transform: "translate(-50%,-50%)" }}>
      <div className={`w-11 h-11 rounded-lg bg-gradient-to-br ${bg[s]} border flex items-center justify-center`}><span className="w-5 h-5 text-white"><I /></span></div>
...
export default function DashboardContent() {
  ...
  const g = health?.components.reduce((a, c) => {
    const k = c.name.includes("Lambda") ? "lambda" : c.name.includes("N8N") ? "n8n" : "vercel";
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
      <div className="text-center"><div className="w-10 h-10 border-2 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto mb-3"></div><p className="text-gray-400 text-sm">Loading...</p></div>
    </div>
  );

  if (err && !health) return (
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
      <div className="glass p-6 rounded-xl text-center max-w-sm">
        <div className="w-12 h-12 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-3"><span className="w-6 h-6 text-red-400"><X /></span></div>
        <p className="text-red-400 font-medium mb-2">Error</p>
        <p className="text-gray-400 text-sm mb-4">{err}</p>
        <button onClick={load} className="px-4 py-2 bg-purple-500/20 rounded-lg text-purple-400 text-sm">Retry</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <header className="glass border-b border-white/10 sticky top-0 z-50">
        <div className="flex items-center justify-between px-5 py-3 max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${health.overall === "green" ? "bg-green-500/20" : health.overall === "yellow" ? "bg-yellow-500/20" : "bg-red-500/20"}`}>
            ...
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-blue-500/20"><span className="w-3.5 h-3.5 text-blue-400"><Cpu /></span><span className="text-xs text-blue-400">{g.lambda?.length || 0}/4</span></div>
            <span className="text-[10px] text-gray-500">{time}</span>
            <button onClick={load} className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10"><span className={`w-4 h-4 text-gray-400 block ${loading ? "animate-spin" : ""}`}><Refresh /></span></button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-5 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-5">
          <div className="glass rounded-xl p-5 flex flex-col items-center"><Gauge v={health.score} c={col[health.overall]} /><div className="mt-2 flex items-center gap-1.5"><Dot s={health.overall} /><span className="text-xs text-gray-400">{health.overall === "green" ? "All OK" : "Issues"}</span></div></div>
          <div className="glass rounded-xl p-5">
            <h3 className="text-xs font-semibold mb-3 flex items-center gap-1.5"><span className="w-3.5 h-3.5 text-cyan-400"><Activity /></span>Stats</h3>
            <div className="grid grid-cols-2 gap-2">
              {[{ l: "Total", v: health.components.length, c: "text-blue-400 bg-blue-500/20" },{ l: "OK", v: health.components.filter(c => c.status === "green").length, c: "text-green-400 bg-green-500/20" },{ l: "Warn", v: health.components.filter(c => c.status === "yellow").length, c: "text-yellow-400 bg-yellow-500/20" },{ l: "Err", v: health.components.filter(c => c.status === "red").length, c: "text-red-400 bg-red-500/20" }].map((s, i) => (
                <div key={i} className={`rounded-lg p-2 text-center ${s.c.split(' ')[1]}`}><div className={`text-xl font-bold ${s.c.split(' ')[0]}`}>{s.v}</div><div className="text-[10px] text-gray-500">{s.l}</div></div>
              ))}
            </div>
          </div>
          <div className="col-span-2 glass rounded-xl p-5">
            <h3 className="text-xs font-semibold mb-3 flex items-center gap-1.5"><span className="w-3.5 h-3.5 text-green-400"><Activity /></span>Trend</h3>
        ...
        {/* Topology */}
        <div className="glass rounded-xl p-5">
          <h3 className="text-xs font-semibold mb-4 flex items-center gap-1.5"><span className="w-3.5 h-3.5 text-purple-400"><Wifi /></span>Network Topology</h3>
          <div className="relative h-56 rounded-lg" style={{ background: "linear-gradient(135deg, rgba(139,92,246,0.05), rgba(6,182,212,0.05))" }}>
            <svg className="absolute inset-0 w-full h-full">
              ...
              {["15%","38%","62%","85%"].map((x, i) => <line key={i} x1="50%" y1="50%" x2={x} y2="15%" stroke="url(#lg)" strokeWidth="1.5" className="conn-line" />)}
              {["25%","50%","75%"].map((x, i) => <line key={i} x1="50%" y1="50%" x2={x} y2="85%" stroke="url(#lg)" strokeWidth="1.5" className="conn-line" />)}
            </svg>
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 via-blue-500 to-cyan-500 flex items-center justify-center shadow-xl shadow-purple-500/40 core-pulse"><span className="w-7 h-7 text-white"><Brain /></span></div>
              <div className="text-center mt-1"><span className="text-[10px] font-bold">ARCHON</span></div>
            </div>
            {g.lambda?.map((c, i) => <Node key={c.name} n={c.name.replace("Lambda: ", "")} s={c.status} I={Cpu} x={["15%","38%","62%","85%"][i]} y="15%" />)}
            <Node n="Vercel" s={g.vercel?.[0]?.status || "green"} I={Globe} x="25%" y="85%" />
            <Node n="AI" s={g.vercel?.find(c => c.name.includes("AI"))?.status || "yellow"} I={Brain} x="50%" y="85%" />
            <Node n="N8N" s={g.n8n?.[0]?.status || "green"} I={Flow} x="75%" y="85%" />
          </div>
        </div>

        {/* Cards */}
        <div className="grid grid-cols-3 gap-5">
          {[{ t: "Lambda", i: Server, c: "text-blue-400", d: g.lambda },{ t: "N8N", i: Flow, c: "text-orange-400", d: g.n8n },{ t: "Vercel", i: Globe, c: "text-cyan-400", d: g.vercel }].map(({ t, i: I, c, d }) => (
            <div key={t} className="glass rounded-xl p-4">
              <h3 className="text-xs font-semibold mb-2 flex items-center gap-1.5"><span className={`w-3.5 h-3.5 ${c}`}><I /></span>{t}</h3>
